...
while True:
        ...
        if control.status == 1:
                ...
                for x in range(0,sl):
                        ...
                        tFile = 'docker exec -i -t -d ubuntu' + str(m) + ' bash -c ' + str("'") + "python /web/traffic_file.py --type " + str(_type) + " --hours " + str(hours) + " --worker " + str(work)  + " --number " + str(number) + " --delay " + str(random.choice(control.delay))  + " --file ubuntu" + str(m)+str("'")
                        _file.write(tFile + '\n')
                        os.system(tRun)
                _file.close()
                time.sleep(hours)
